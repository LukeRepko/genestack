...
# Create namespace with Helm labels so Helm can adopt it during install
echo "==> Ensuring namespace '${SERVICE_NAMESPACE}' exists with Helm ownership"
if ! kubectl get ns "${SERVICE_NAMESPACE}" >/dev/null 2>&1; then
    kubectl create ns "${SERVICE_NAMESPACE}"
fi
# Add Helm ownership labels/annotations so Helm can manage it
kubectl label ns "${SERVICE_NAMESPACE}" app.kubernetes.io/managed-by=Helm --overwrite
kubectl annotate ns "${SERVICE_NAMESPACE}" meta.helm.sh/release-name="${SERVICE_NAME}" --overwrite
kubectl annotate ns "${SERVICE_NAMESPACE}" meta.helm.sh/release-namespace="${SERVICE_NAMESPACE}" --overwrite
